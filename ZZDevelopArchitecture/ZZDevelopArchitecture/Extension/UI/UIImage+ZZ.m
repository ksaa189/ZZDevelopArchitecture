...
@interface ZZImageCache : ZZMemoryCache __AS_SINGLETON

@end

@implementation ZZImageCache __DEF_SINGLETON

@end

DUMMY_CLASS(UIImage_ZZ);
...
+ (UIImage *)imageNamed:(NSString *)name useCache:(BOOL)useCache
{
    if (YES == useCache)
    {
        ...
        {
            UIImage * image = [[ZZImageCache sharedInstance] objectForKey:[NSString stringWithFormat:@"Application-%@",name]];
            if (image)
            {
                return image;
            ...
            else
            {
                ...
                if (image)
                {
                    [[ZZImageCache sharedInstance] setObject:image forKey:[NSString stringWithFormat:@"Application-%@",name]];
                }
