...
#import "ZZKeyboardHelper.h"
#import "UIWindow+ZZ.h"

@interface ZZKeyboardHelper ()
{
...
@implementation ZZKeyboardHelper __DEF_SINGLETON

-(id) init
{
    if (self = [super init])
    {
        _keyboardDistanceFromTextField = ZZKeyboardHelper_DefaultDistance;
        _isEnabled = NO;
...
- (void)disableKeyboardHelper
{
    ...
    {
        self.isEnabled = NO;
        self.keyboardDistanceFromTextField = ZZKeyboardHelper_DefaultDistance;
...
- (void)setRootViewFrame:(CGRect)frame
{
    ...
    UIViewController *controller = [UIWindow KeyWindowTopMostController];
...
- (void)keyboardDidShow:(NSNotification*)aNotification
{
    ...
    switch ([UIWindow KeyWindowTopMostController].interfaceOrientation)
    {
...
- (void)adjustFrameWithDuration:(CGFloat)aDuration {
    ...
    UIViewController *rootController = [UIWindow KeyWindowTopMostController];
    ...
    //Special case.
    if ([[UIWindow KeyWindowTopMostController] modalPresentationStyle] == UIModalPresentationFormSheet ||
        [[UIWindow KeyWindowTopMostController] modalPresentationStyle] == UIModalPresentationPageSheet)
    {
        //Positive or zero.
        if (move>=0)
        {
...
- (void)addPreviousNextDoneOnKeyboardWithTarget:(id)target previousAction:(SEL)previousAction nextAction:(SEL)nextAction doneAction:(SEL)doneAction
{
    ...
    ZZSegmentedNextPrevious *segControl = [[ZZSegmentedNextPrevious alloc] initWithTarget:target previousSelector:previousAction nextSelector:nextAction];
    //
...
@implementation ZZSegmentedNextPrevious
...
- (void)segmentedControlHandler:(ZZSegmentedNextPrevious*)sender
{
