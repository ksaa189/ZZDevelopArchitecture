...
static NSString *const AOPAspectCurrentObjectKey = @"ZZAOPAspectCurrentObjectKey";

@interface ZZAOP ()

@property (nonatomic, strong) dispatch_queue_t synchronizerQueue;
@property (nonatomic, copy) ZZAOP_block methodInvoker;
...
@implementation ZZAOP __DEF_SINGLETON
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _synchronizerQueue = dispatch_queue_create("com.ZZ.aopQueue", DISPATCH_QUEUE_SERIAL);
...
- (NSString *)registerClass:(Class)aClass withSelector:(SEL)aSelector type:(AOPAspectInspectorType)type usingBlock:(ZZAOP_block)block {
...
- (NSString *)saveInterceptorBlock:(ZZAOP_block)block withClass:(Class)aClass selector:(SEL)aSelector type:(AOPAspectInspectorType)type
{
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                for (NSDictionary *dic in [NSArray arrayWithArray:interceptorArray])
                {
                    ...
                    if ([[self identifierWithClass:aClass selector:selector dictionary:dic] isEqualToString:identifier])
                    {
                        ...
                        {
                            [self saveInterceptorBlock:_methodInvoker
                                             withClass:aClass
                                              selector:selector
                                                  type:i];
                        }
...
- (void)executeInterceptorsWithClass:(Class)aClass selector:(SEL)aSelector invocation:(NSInvocation *)anInvocation {
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            ZZAOP_block block = [[interceptor allValues] lastObject];
            block(anInvocation);
        }
    }
...
- (id)baseClassForwardingTargetForSelector:(SEL)aSelector
{
    ...
    {
        SEL fSelNew = [[ZZAOP sharedInstance] extendedSelectorWithClass:[self class]
                                                               selector:@selector(forwardingTargetForSelector:)];
        return AOPAspectAction([ZZAOP sharedInstance], fSelNew);
    }
    
    [[ZZAOP sharedInstance] setCurrentObject:self];
    
    return [ZZAOP sharedInstance];
}
